...
# This file was auto-generated by hosts-calc.service
...
EOF
ip -j addr | jq -r '.[] | select(.link_type != "loopback") | .addr_info[] | select(.scope != "host") | .local' | while read -r PUB_IP_ADDR; do
tee -a /etc/hosts <<EOF

\$(printf "%-14s  %20s  %20s" "\$PUB_IP_ADDR" "\$FQDNAME" "\$HOSTNAME")
\$(printf "%-14s  %20s  %20s" "\$PUB_IP_ADDR" "router.internal" "router")
\$(printf "%-14s  %20s  %20s" "\$PUB_IP_ADDR" "gateway.internal" "gateway")
EOF
done
EOS
...
WantedBy=multi-user.target
EOF
tee /etc/systemd/system/hosts-calc.timer <<EOF
[Timer]
OnBootSec=30
...
systemctl enable dnsmasq ntpd.timer step-ca hosts-calc.timer
